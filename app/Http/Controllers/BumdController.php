...
class BumdController extends Controller
{
    ...
    public function update(Request $request, Bumd $bumd)
    {
        try {
            ...
            if ($request->hasFile('logo')) {
                $storedPath = $request->file('logo')->store('bumd', 'public');
                $bumdData['logo'] = Storage::url($storedPath); // MODIFIKASI

                // ... (Logika hapus logo lama Anda sudah benar) ...
            ...
            } elseif (array_key_exists('logo', $bumdData) && $bumdData['logo'] === null) { // MODIFIKASI
                // ... (Logika hapus logo lama Anda sudah benar) ...
                if ($oldLogoPath && Str::startsWith($oldLogoPath, '/storage/')) {
                    $oldRel = Str::replaceFirst('/storage/', '', $oldLogoPath);
                    if (Storage::disk('public')->exists($oldRel)) {
                ...
                // BARU: Pastikan logo tidak di-update jika tidak ada file baru ATAU tidak dikosongkan
                // Ini mencegah $validated['logo'] (yang mungkin berisi string path lama) menimpa dirinya sendiri
                unset($bumdData['logo']);
            }
